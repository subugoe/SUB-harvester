                ...
                <xsl:when test="dc:title and (string-length(dc:title) &gt; 0)">
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="string-length(dc:title) &gt; 150">
                            <xsl:value-of select="normalize-space(concat(substring(dc:title, 1, 147), '...'))"/>
                        </xsl:when>
...
-->
		...
		<xsl:if test="string-length($language) &lt; 3">
			<field name="language">
				<xsl:call-template name="languageCodeConverter">
					<xsl:with-param name="languageCode" select="$language"/>
				</xsl:call-template>
			</field>
		</xsl:if>
	</xsl:template>
	...
	<xsl:template match="dc:identifier | dc:relation">
		...
		<xsl:choose>
			<xsl:when test="substring($identifier, 1, 4) = 'doi:'">
				<field name="doi">
					<xsl:value-of select="substring($identifier, 5)"/>
				</field>
			</xsl:when>
			<xsl:when test="substring($identifier, 1, 18) = 'http://dx.doi.org/'">
				<field name="doi">
					<xsl:value-of select="substring($identifier, 19)"/>
				</field>
			</xsl:when>
			<xsl:when test="substring($identifier, 1, 7) = 'http://'
							or substring($identifier, 1, 8) = 'https://'
							or substring($identifier, 1, 6) = 'ftp://'">
				<field name="url">
		...
		<xsl:if test="$firstItem">
			<xsl:if test="(not($minLength) or string-length($firstItem) = $minLength)
				...
				<field>
					...
					<xsl:value-of select="normalize-space($firstItem)"/>
				</field>
			</xsl:if>
		</xsl:if>
