...
<xsl:stylesheet
    ...
    <xsl:template match="oai:ListRecords/oai:record" mode="add">
        ...
        <doc>
            ...
            <xsl:apply-templates select="oai:metadata/pica:record/*"/>
        </doc>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="pica:datafield">
    	<xsl:variable name="field" select="translate(@tag, '@', '_')"/>
    	<field name="{concat('d', $field)}">
	    	<xsl:for-each select="pica:subfield">
		    	<xsl:value-of select="."/>
		    	<xsl:text> </xsl:text>
	    	</xsl:for-each>
    	</field>
    	...
    	<xsl:for-each select="pica:subfield">
    		<field name="{concat('d', $field, 's', @code)}">
    			<xsl:value-of select="."/>
    		</field>
    	</xsl:for-each>
    </xsl:template>
